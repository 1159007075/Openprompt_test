...
class Verbalizer(nn.Module):
    ...
    Args:
        tokenizer (:obj:`PreTrainedTokenizer`): A tokenizer to appoint the vocabulary and the tokenization strategy. 指定词汇表和标记化策略的标记化器
    ...
    def label_words(self,):
        ...
        E.g. if we want to establish a projection in sentiment classification: positive :--> {`wonderful`, `good`},
        in this case, `wonderful` and `good` are label words.
        标签词是指标签投射的词汇表中的单词。
        例如，如果我们想在情感分类中建立一个投影：正面：-->`{`wood`，`good`}，在这种情况下，`wood'和`good'是标签词
        '''
    ...
    def _match_label_words_to_label_ids(self, label_words): # TODO newly add function after docs written # TODO rename this function
        ...
        对verbalizer的标签词dict进行排序，以匹配类的标签顺序
        """
    ...
    def aggregate(label_words_logits: torch.Tensor) -> torch.Tensor:
        ...
        要将多个标签词上的 logits 聚合到标签的 logits 基本聚合器: 每个标签词的 logit 到标签的 logit 的平均值可以在高级语言表达器中重新实现
        Args:
    ...
    def project(self,
        ...
        他的方法接收形状为`[batchsize，vocb_size]``的输入logits，
        并使用该描述器的参数将整个vocb上的logits投射到标签单词的logits中。
    ...
    def handle_multi_token(self, label_words_logits, mask):
        ...
        支持多个方法来处理由标记生成器生成的多个标记。
        如果标记化的某些部分没有意义，我们建议使用“ first”或“ max”。
        可以广播到三维张量。
