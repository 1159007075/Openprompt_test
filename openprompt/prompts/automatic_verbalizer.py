...
class AutomaticVerbalizer(Verbalizer):
    ...
    def project(self,
        ...
        当这个Verbalizer没有执行optimize（）时，
        它没有``label_words_ids``，因此将给出随机预测，并且应该与模型没有关联以给出（未命中前导）梯度。
    ...
    def optimize_to_initialize(self):
        ...
        is small.
        这是一个批处理级优化。
        如果像普通的梯度下降优化器一样在批处理级别使用，结果可能不太令人满意，因为如果批处理大小很小，精确的示例（即probs_buffer和labels_buffer）不够。
        """
