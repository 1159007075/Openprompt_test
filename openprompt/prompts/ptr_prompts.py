...
class PTRTemplate(PtuningTemplate):
    """
    Args:
        model (:obj:`PreTrainedModel`): The pre-trained language model for the current prompt-learning task. 预训练语言模型
        tokenizer (:obj:`PreTrainedTokenizer`): A tokenizer to appoint the vocabulary and the tokenization strategy. 词汇表标记器
        text (:obj:`Optional[List[str]]`, optional): manual template format. Defaults to None. 手动模板格式
        soft_token (:obj:`str`, optional): The special token for soft token. Default to ``<soft>`` 用于软令牌的特殊令牌
        placeholder_mapping (:obj:`dict`): A place holder to represent the original input text. Default to ``{'<text_a>': 'text_a', '<text_b>': 'text_b'}`` 原始输入文本的占位符
    """
    def __init__(self,
...
class PTRVerbalizer(Verbalizer):
    ...
    不同的“ < mask >”标记有不同的标签字。最终的标签是由这些标签词使用逻辑规则联合预测的
    ...
    def on_label_words_set(self):
        """
        Prepare One2one Verbalizer for each `<mask>` separately
        分别为每个`<mask>`准备一个一对一的Verbalizer
        """
        ...
        for words in self.label_words:
            if len(words) != self.num_masks:
                raise ValueError("不同类的mask标记长度不一致")
        self.sub_labels = [
